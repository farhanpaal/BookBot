...
from database.ia_filterdb import col, sec_col, db as vjdb, sec_db, get_file_details, get_search_results, get_bad_files, fetch_google_titles
...
@Client.on_callback_query(filters.regex(r"^libsearch_"))
async def handle_libsearch_callback(client, callback_query):
    """Handle Library Search button clicks"""
    try:
        ...
        await callback_query.answer("Searching in Library...")
        reply_msg = callback_query.message
        await reply_msg.edit_text(f"🔍** Doing a deep search for **'{movie}'** in Library**...")

        try:
            # Perform LibGen search
            ...
            if results:
                ...
                await reply_msg.edit_text(f"⚠️ No LibGen results found for '{movie}'.")
                
        except Exception as e:
            logger.error(f"LibGen search error: {e}")
            await reply_msg.edit_text(
                f"**⚠️ Library Search Failed**\nError: `{e}`",
                parse_mode=enums.ParseMode.MARKDOWN
            )
            
    except Exception as e:
        logger.error(f"Library search callback error: {e}")
...
def get_file_type(filename: str) -> str:
    ...
    # Clean filename: convert to uppercase
    cleaned = filename.upper()

    # Check if the cleaned filename ends with any of the valid extensions (longest first)
...
async def pm_text(bot, message):
    ...
    await db.update_top_messages(user_id, content)
    if PM_SEARCH == True:
        ai_search = True
   ...
   # if not search:
    ...
    if settings['button']:
        btn = [
            [
                InlineKeyboardButton(
                   text=f"{get_file_type(file['file_name'])}~{get_size(file['file_size'])} - {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}", callback_data=f'{pre}#{file["file_id"]}'
                ),
            ]
            for file in files
        ]
        ...
        btn.insert(0, [
            ...
            InlineKeyboardButton("Fᴏʀᴍᴀᴛᴇs", callback_data=f"years#{key}")
        ])
    else:
        btn = []
        # btn.insert(0, 
...
async def advantage_spoll_choker(bot, query):
    ...
    orig = query.message.reply_to_message or query.message
    movies = SPELL_CHECK.get(orig.id)

    if not movies or movies is None:
        # nothing to pick from—session expired or never set
        return await query.answer(
            "❌ Spell-check session expired. Please try again.",
            show_alert=True
        )
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="sᴇʟᴇᴄᴛ ʏᴏᴜʀ Fᴏʀᴍᴀᴛᴇ", callback_data="ident"
            )
        ],
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        missing = await is_subscribed(client, query, AUTH_CHANNEL)
        if missing: 
     ...
     #   if not search:
      ...
      #      return
        ...
        ident, from_user_str = query.data.split("#")
        from_user = int(from_user_str)  # Convert to integer
        btn = [[
        ...
        if query.from_user.id in ADMINS:  # Ensure ADMINS has integers
            try:
                user = await client.get_users(from_user)
                # Edit the message
                await query.message.edit_text(f"<b><s>{query.message.text}</s></b>", reply_markup=InlineKeyboardMarkup(btn))
                await query.answer("Set to Uploaded!")
                # Send message to user
                try:
                    await client.send_message(
                        chat_id=from_user,
                        text=f"<b>Hey {user.mention}, Your request is uploaded!</b>",
                        reply_markup=InlineKeyboardMarkup(btn2)
                    )
                except UserIsBlocked:
                    await client.send_message(
                        chat_id=SUPPORT_CHAT_ID,
                        text=f"User blocked the bot. Notification failed for {user.mention}."
                    )
                except Exception as e:
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hᴇʏ {user.mention}, Yᴏᴜʀ ʙᴏᴏᴋs ɪs ᴀʟʀᴇᴀᴅʏ ᴀᴠᴀɪʟᴀʙʟᴇ ɪɴ ᴏᴜʀ ʙᴏᴛ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ᴀɢᴀɪɴ ɪɴ ᴏᴜʀ ʙᴏᴛ </b>", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hᴇʏ {user.mention}, Yᴏᴜʀ ʙᴏᴏᴋs ɪs ᴀʟʀᴇᴀᴅʏ ᴀᴠᴀɪʟᴀʙʟᴇ ᴏɴ ᴏᴜʀ ʙᴏᴛ's ᴅᴀᴛᴀʙᴀsᴇ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.\n\nNᴏᴛᴇ: Tʜɪs ᴍᴇssᴀɢᴇ ɪs sᴇɴᴛ ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ'ᴠᴇ ʙʟᴏᴄᴋᴇᴅ ᴛʜᴇ ʙᴏᴛ. Tᴏ sᴇɴᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ PM, Mᴜsᴛ ᴜɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ.</b>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/Self_Improvement_AudioBook_Bot")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            find = search.split(" ")
            search = ""
            removes = ["in","upload", "series", "full", "horror", "thriller", "mystery", "print", "file","book","novel","ebook", "pdf", "epub", "mobi", "audiobook", "audio", "mp3", "mp4", "video", "movie", "movies", "film", "films", "show", "shows"]
            for x in find:
                if x in removes:
                    continue
                else:
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="🔍Get More Results From Library", callback_data=f"libsearch_{key}")]
        )
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap+="<b>\n\n<u>🍿 Your Audiobook Or Book Files 👇</u></b>\n"
            for file in files:
                cap += f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}\n</a></b>"
    else:
        if settings["button"]:
            cap = f"<b>Tʜᴇ Rᴇꜱᴜʟᴛꜱ Fᴏʀ ☞ {search}\n\nRᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞ {message.from_user.mention}\n\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ ☞ {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ ☞ :<a href='{CHNL_LNK}'>Variable Tribe</a>  \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
        else:
            cap = f"<b>Tʜᴇ Rᴇꜱᴜʟᴛꜱ Fᴏʀ ☞ {search}\n\nRᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞ {message.from_user.mention}\n\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ ☞ {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ ☞ : <a href='{CHNL_LNK}'>Variable Tribe</a>  \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
            cap+="<b><u>🍿 Your Audiobook Or Book Files 👇</u></b>\n\n"
            for file in files:
