...
MAX_TRENDS = 10  # Max queries to store (adjust as needed)
...
class Database:
    ...
    def __init__(self, uri, database_name):
        ...
        self.top_messages = self.db.top_messages
    ...
    async def update_top_messages(self, user_id: int, message_text: str):
        # Update count if query exists
        doc = await self.top_messages.find_one({"message": message_text})
        if doc:
            await self.top_messages.update_one(
                {"message": message_text},
                {"$inc": {"count": 1}}
            )
        # Add new query if it doesn't exist
        else:
            await self.top_messages.insert_one({
                "message": message_text,
                "count": 1,
                "created_at": datetime.datetime.now()
            })
            
            # Remove oldest least-popular query if over limit
            total = await self.top_messages.count_documents({})
            if total > MAX_TRENDS:
                # Find the oldest doc with the smallest count
                pipeline = [
                    {"$sort": {"count": 1, "created_at": 1}},  # Lowest count + oldest first
                    {"$limit": 1}
                ]
                async for doc in self.top_messages.aggregate(pipeline):
                    await self.top_messages.delete_one({"_id": doc["_id"]})

    async def get_top_messages(self, limit: int = 30) -> list[str]:
        cursor = self.top_messages.find().sort("count", -1).limit(limit)
